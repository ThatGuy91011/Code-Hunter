...
int main()
{
	...
	for (int i = 0; i < textToAnalyze.length(); ++i)
	{
		if (textToAnalyze[i] == 'a' || textToAnalyze[i] == 'e' || textToAnalyze[i] == 'i' ||
			...
			textToAnalyze[i] == 'U')
		{
			++vowels; //Line was subtracting from "vowels" when it should be adding
		}
		else if ((textToAnalyze[i] >= 'a' && textToAnalyze[i] <= 'z') || (textToAnalyze[i] >= 'A' && textToAnalyze[i] <= 'Z'))
		{
			++consonants;  //Line was accidentally commented
		}
	...
	//These variables were declared at the beginning of the code, but it would save on space to merge the declaration and these lines together
	int lengthOfStringSubmittedForAnalysis = textToAnalyze.length();
	
	int checkSum = unknownCharacters + vowels + consonants + digits + spaces;
